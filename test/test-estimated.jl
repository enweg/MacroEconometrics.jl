...
@testset "FrequentistEstimated" begin
    v = rand(1)
    fe_1d = FrequentistEstimated(v, v, v, nothing)
    v = rand(3, 3)
    fe_2d = FrequentistEstimated(v, v, v, nothing)
    v = rand(3, 3, 3)
    fe_3d = FrequentistEstimated(v, v, v, nothing)

    v = rand(2, 2)
    # Shoudl fail because ci_upper < ci_lower
    @test_throws AssertionError FrequentistEstimated(v, v, v .- 1, nothing)
    # Should fail because ci_lower > value
    @test_throws AssertionError FrequentistEstimated(v, v .+ 1, v .+ 1, nothing)
    # Should fail because dimensions do not match
    @test_throws AssertionError FrequentistEstimated(v, v, vcat(v, v), nothing)
    @test_throws AssertionError FrequentistEstimated(v, vcat(v, v), vcat(v, v), nothing)
    @test_throws AssertionError FrequentistEstimated(v, vcat(v, v), v, nothing)
